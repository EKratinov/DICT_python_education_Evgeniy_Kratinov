...
def validate_args(args):
    if "type" not in args or "interest" not in args or args["type"] not in ["annuity", "diff"]:
        print("Incorrect parameters")
        return False
...
def process_args():
    ...
    if args["type"] == "diff":
        if "payment" in args or "principal" not in args or "periods" not in args:
            print("Incorrect parameters")
            return
        calculate_diff(float(args["principal"]), int(float(args["periods"])), float(args["interest"]))
    else:
